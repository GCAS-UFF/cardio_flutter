...
class EntityCard extends StatefulWidget {
  ...
  final List<Activity<dynamic>> activityList;
...
class _EntityCardState extends State<EntityCard> {
  ...
  List<Activity<dynamic>> _recomendedActivity = [];
  List<Activity<dynamic>> _doneActivity = [];

  // @override
  // void initState() {
  //   super.initState();
  //   for (Activity<dynamic> activity in widget.activityList) {
  //     if (activity.value == Keys.ACTIVITY_RECOMENDED)
  //       _recomendedActivity.add(activity);
  //     else
  //       _doneActivity.add(activity);
  //   }
  // }
...
*/
  Widget _buildParameterItem(BuildContext context, MapEntry entry) {
    return RecomendationDetailTile(
      title: entry.key,
      content: entry.value,
    );
  }
